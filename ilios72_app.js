...
async function pullClientDataFromSupabase(opts={}){
  ...
  try{
    ...
    clients=(c||[]).map(fromSnakeRow); sv(K.C,clients);
    holdings=(h||[]).map(fromSnakeRow); sv(K.H,holdings);
    txns=(t||[]).map(fromSnakeRow); sv(K.T,txns);
    uploads=(u||[]).map(fromSnakeRow); sv(K.UP,uploads);
    _lastSyncedIds={
