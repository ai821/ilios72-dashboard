...
function sv(k,d){
  ...
  if(table&&!_suppressSupaPush)pushArrayToSupabase(table,d);
}
...
async function pullClientDataFromSupabase(opts={}){
  ...
  try{
    const [c,h,t,u]=await Promise.all([
    ...
    if(!silent)toast('Client data refreshed from cloud ✓');
  }catch(e){
    _suppressSupaPush=false;
    console.error('pullClientDataFromSupabase failed:',e);
    if(!silent)toast('Could not load client data from Supabase — showing this device\'s local copy','r');
  }
}
