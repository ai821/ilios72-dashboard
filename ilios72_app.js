...
// Same normalization the sync script (nse_symbol_sync.py) applies to NSE's
// official company names, so a lookup here actually matches a stored row.
function normalizeNseKey(name){
  let s=(name||'').toUpperCase().trim();
  s=s.replace(/\bLIMITED\b\.?$/,'').trim();
  s=s.replace(/\bLTD\b\.?$/,'').trim();
  s=s.replace(/[^A-Z0-9]/g,'');
  return s;
}

// Checks NSE's own official name-to-symbol list (synced weekly into
// Supabase) — a real, verified answer straight from the exchange, tried
// before ever falling back to live search or an AI guess.
async function lookupNseMasterList(stockName){
  try{
    const key=normalizeNseKey(stockName);
    if(!key)return null;
    const rows=await sb.from('nse_symbol_master').select('*').eq('id',key).limit(1);
    if(rows&&rows.length)return rows[0];
    return null;
  }catch(e){
    console.error('NSE master list lookup failed (non-fatal, falling back):',e);
    return null;
  }
}


// For stocks with NO known ticker mapping yet, and not found in NSE's own
// master list either: search Angel One's own NSE symbol list using the
// actual company name (progressively shortened), score results by how many
...
async function fetchLivePrices(){
  ...
  async function fetchOne(stockName,nseSym,attempt,cacheRetried){
    ...
    if(!isKnownSymbolMapping(stockName)){
      ...
      // First, check NSE's own official name-to-symbol list — a real,
      // verified answer straight from the exchange. If it has this
      // company, we still need Angel One's own instrument token for the
      // actual price fetch, so resolve that for the confirmed-correct
      // symbol (not a guessed one).
      const nseMatch=await lookupNseMasterList(stockName);
      if(nseMatch&&nseMatch.symbol){
        const confirmedSym=nseMatch.symbol+'-EQ';
        const tokenResult=await resolveSymbolToken(confirmedSym,headers,tokenCache);
        if(tokenResult){
          found={tradingsymbol:confirmedSym,symboltoken:tokenResult.token,exchange:tokenResult.exchange};
          source='nse-master-list';
        }
      }

      if(!found)found=await smartSearchTicker(stockName,headers);
      ...
      if(found){
        ...
        autoDiscovered++;
        if(source==='nse-master-list')console.log(`NSE master list match for "${stockName}" → ${discoveredTicker}`);
        else if(aiAssisted)console.log(`AI-assisted ticker match for "${stockName}" → ${discoveredTicker} (verified against Angel One)`);
      }else{
