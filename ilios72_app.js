...
function renderHoldings(){
  ...
  const isSuspicious = s => s && s.length>60;
  ...
  tbody.innerHTML=h.map(x=>{
    ...
    const suspicious = isSuspicious(x.stock);
    return `<tr${suspicious?' style="background:rgba(239,68,68,0.06)"':''}>
      ...
      <td style="max-width:260px" title="${suspicious ? 'This looks like corrupted import data, not a real stock name — recommend deleting this row.' : ''}">
      ...
      <td style="text-align:center"><button class="btn btn-d btn-sm" style="padding:2px 8px;font-size:10px" onclick="deleteHolding('${x.id}')" title="Delete this holding">✕</button></td>
    </tr>`;
  }).join('');
}
