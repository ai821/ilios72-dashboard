...
// and Transactions cards. Shares the same real PDF.js extraction as the
// factsheet uploader (handleManualPdfUpload) — no more misreading binary
// PDF bytes as plain text.
async function handlePdfToPaste(event, targetElId){
  ...
  try {
    ...
    for(let pageNum=1; pageNum<=pdf.numPages; pageNum++){
      targetEl.textContent = `⏳ Extracting page ${pageNum} of ${pdf.numPages}...`;
...
async function handleManualPdfUpload(event){
  ...
  try {
    if(typeof pdfjsLib === 'undefined'){
      throw new Error('PDF reader library did not load — check your internet connection and refresh the page');
    }
    pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';
    ...
    let fullText = '';
    for(let pageNum=1; pageNum<=pdf.numPages; pageNum++){
      statusEl.textContent = `⏳ Extracting page ${pageNum} of ${pdf.numPages}...`;
      const page = await pdf.getPage(pageNum);
      const textContent = await page.getTextContent();
    ...
    if(!fullText){
      ...
      statusEl.textContent = `✓ PDF loaded — ${file.name} (${pdf.numPages} page${pdf.numPages!==1?'s':''}, ${Math.round(fullText.length/1024)}KB text extracted)`;
    }
  } catch(e){
    console.error('PDF extraction failed:', e);
