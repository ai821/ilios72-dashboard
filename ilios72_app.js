...
const APMI_PROXY='http://localhost:8420';
...
async function refreshFromAPMI(){
  ...
  try{
    const resp=await fetch(APMI_PROXY+'/refresh');
    if(!resp.ok)throw new Error('Proxy returned '+resp.status);
    const data=await resp.json();
    let updated=0,failed=0;
    (data.results||[]).forEach(r=>{
      if(r.status!=='OK'){failed++;return;}
      ...
      p.updatedAt=r.updatedAt||new Date().toISOString();
      updated++;
    });
    sv(K.PMS,pmsList);
    localStorage.setItem('i72_apmi_sync',data.syncedAt||new Date().toISOString());
    updateApmiSyncLabel();
    renderAll();
    toast(updated+' PMS updated from APMI'+(failed?(', '+failed+' failed'):'')+' ✓');
  }catch(e){
    toast('Could not reach APMI proxy — is apmi_proxy.py running? ('+e.message+')');
  }finally{
...
async function analyzeFactsheet(pmsId) {
  const provider = document.getElementById('aiProvider')?.value || 'mistral';
  const apiKey   = document.getElementById('aiApiKey')?.value?.trim();
  if(!apiKey){
    toast('Enter your ' + (provider==='mistral'?'Mistral':'Claude') + ' API key first');
    return;
  }
  ...
  badgeEl.textContent = (provider==='mistral'?'Mistral':'Claude') + ' — Generating...';
  contentEl.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px">🤖 '
    + (provider==='mistral'?'Mistral AI':'Claude') + ' is analysing '
...
Output pure HTML only. Do not repeat raw numbers — interpret them.`;
  ...
  try {
    ...
    if(provider === 'mistral') {
      // -- Mistral API (free tier, no credit card) --
      ...
      html = json.choices?.[0]?.message?.content || 'No analysis returned.';
      ...
      html = json.content?.[0]?.text || 'No analysis returned.';
    }
    ...
    contentEl.innerHTML =
      ...
      + '<span class="badge bo" style="font-size:9px">'+(provider==='mistral'?'Mistral AI':'Claude')+' — '+(p?.name||'')+' Analysis</span>'
    ...
    contentEl.innerHTML = '<div style="color:var(--red);padding:12px">⚠ Error: '+e.message+'<br><br>'
      +(provider==='mistral'
        ? 'Check your Mistral key at console.mistral.ai — make sure you selected the Experiment plan in Billing.'
        : 'Check your Claude API key at console.anthropic.com')
      +'</div>';
    badgeEl.textContent = 'Error';
    toast('Analysis failed — check API key');
  }
}
...
function receiveFactsheetFromN8n(payload){
  ...
  // Auto-analyse if API key is present
  const key = document.getElementById('aiApiKey')?.value?.trim();
  if(key) openFsiAnalysis(entry.id);
}
...
async function runFactsheetAI(entry, aiDiv, aiBadge, structDiv, holdingsElId, clientElId, entryId){
  const provider = document.getElementById('aiProvider')?.value || 'mistral';
  const apiKey   = document.getElementById('aiApiKey')?.value?.trim();
  if(!apiKey){ aiDiv.innerHTML='<div style="color:var(--red)">⚠ Enter your '+provider+' API key at the top of this tab</div>'; aiBadge.textContent='No Key'; return; }
  ...
  const textSample = (entry.extractedText||'').slice(0,4000);

  const extractionPrompt = `You are a professional PMS analyst. Below is text extracted from a ${entry.pmsName} factsheet PDF.
...
Respond with a JSON object ONLY (no markdown, no explanation) in this exact format:
{
  ...
  "returns": [
    {"period": "1 Month", "fund": "", "benchmark": "", "alpha": ""},
    {"period": "3 Month", "fund": "", "benchmark": "", "alpha": ""},
    {"period": "6 Month", "fund": "", "benchmark": "", "alpha": ""},
    {"period": "1 Year", "fund": "", "benchmark": "", "alpha": ""},
    {"period": "2 Year", "fund": "", "benchmark": "", "alpha": ""},
    {"period": "3 Year", "fund": "", "benchmark": "", "alpha": ""},
    {"period": "5 Year", "fund": "", "benchmark": "", "alpha": ""},
    {"period": "Since Inception", "fund": "", "benchmark": "", "alpha": ""}
  ],
  "topHoldings": [
...
For holdingsImpact.stocksInPortfolio, cross-reference the top holdings from the factsheet with: [${holdingsCtx}]
For clientImpact, assess each client based on their investment amount and risk profile.
Fill every field. Use actual numbers from the factsheet text where available.`;
  ...
  try {
    ...
    if(provider==='mistral'){
      const resp = await fetch('https://api.mistral.ai/v1/chat/completions',{
        ...
        body:JSON.stringify({model:'mistral-small-latest',messages:[{role:'user',content:extractionPrompt}],max_tokens:2000,temperature:0.1})
      });
      const json = await resp.json();
      if(json.error) throw new Error(json.error.message||JSON.stringify(json.error));
      ...
      const resp = await fetch('https://api.anthropic.com/v1/messages',{
        ...
        body:JSON.stringify({model:'claude-sonnet-4-6',max_tokens:2000,messages:[{role:'user',content:extractionPrompt}]})
      });
      const json = await resp.json();
      if(json.error) throw new Error(json.error.message);
...
function renderStructuredFactsheet(data, pmsId){
  ...
  return `
    ...
    <!-- Top Holdings + Sector + Cap (3 col) -->
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:14px">
      ${holdings.length?`
      <div class="card" style="padding:0;overflow:hidden">
        <div style="padding:12px 14px;border-bottom:1px solid var(--border)"><div class="sec" style="margin:0">🏦 Top Holdings</div></div>
        <table>
          <thead><tr><th>#</th><th>Stock</th><th style="text-align:right">Wt%</th></tr></thead>
          <tbody>${holdings.slice(0,10).map(h=>`<tr>
            <td style="color:var(--muted);font-size:10px">${h.rank}</td>
            <td style="font-size:11px;font-weight:500">${h.stock}<br><span style="font-size:9px;color:var(--muted)">${h.sector||''}</span></td>
