...
async function extractPdfTextLayoutAware(file, onProgress){
  ...
  for(let pageNum=1; pageNum<=pdf.numPages; pageNum++){
    ...
    // Group items into rows by Y position (rounded, so tiny baseline
    // jitter within the same visual row doesn't split it into two rows).
    const rowMap = {};
    ...
    // PDF Y-coordinates increase upward, so sort rows top-to-bottom
    // (descending Y) to match natural reading order.
