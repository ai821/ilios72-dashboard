...
(async function checkAuth() {
  try {
    ...
    // Pull shared client/holdings/transactions/uploads data — this is what
    // makes one employee's upload visible on every other employee's screen.
    if (typeof pullClientDataFromSupabase === 'function') pullClientDataFromSupabase({ silent: true });
  } catch(e) {
...
// ==================================================
// SUPABASE SYNC — clients / holdings / transactions / uploads
// --------------------------------------------------
// Previously these four lived ONLY in localStorage(K.C/K.H/K.T/K.UP), so
// anything one employee uploaded stayed on that one browser/device and
// never showed up for anyone else. sv() now also pushes to the matching
// Supabase table (fire-and-forget) and pullClientDataFromSupabase() loads
// the shared cloud copy on every page open. localStorage is kept as a
// same-device instant-load cache only — Supabase is the source of truth.
//
// Column names are auto-converted camelCase<->snake_case (e.g. clientId
// <-> client_id), matching the convention already used by pms_list
// (r1m, updated_at, etc). If your actual `clients`/`holdings` table columns
// don't follow this pattern, tell Claude and this mapping can be adjusted —
// see the accompanying message for the exact column list this expects.
// ==================================================
const SUPA_TABLE={[ 'i72_c']:'clients',['i72_h']:'holdings',['i72_t']:'transactions',['i72_up']:'uploads'};
// Blob-shaped data (objects/arrays that aren't a clean per-row table) —
// synced whole, as a single jsonb row per key, via the shared_state table.
const SHARED_BLOB_KEYS=new Set(['i72_analysis','i72_pms_sector_alloc','i72_tk','i72_snaps','i72_fs_state','i72_fsi_manual']);
let _suppressSupaPush=false;                    // true while a pull is writing local state back
let _lastSyncedIds={clients:new Set(),holdings:new Set(),transactions:new Set(),uploads:new Set()};

async function pushBlobToSupabase(key,value){
  if(typeof sb==='undefined')return;
  try{
    await sb.from('shared_state').upsert({key,value,updated_at:new Date().toISOString()});
  }catch(e){
    console.error(`Supabase sync failed for shared_state:${key}`,e);
    toast(`⚠️ Could not sync "${key}" to the cloud — only saved on this device right now`,'r');
  }
}
...
function toSnakeRow(o){const out={};Object.keys(o).forEach(k=>out[camelToSnake(k)]=o[k]);return out}
function fromSnakeRow(o){const out={};Object.keys(o).forEach(k=>out[snakeToCamel(k)]=o[k]);return out}

// Explicit overrides for the handful of fields whose real Supabase column
// name doesn't match the standard camelCase<->snake_case conversion
// (confirmed against actual schema via information_schema.columns —
// don't guess these again, check the real schema first if adding more).
const FIELD_RENAME_TO_DB={ clients:{date:'start_date'}, holdings:{addedAt:'created_at'} };
const FIELD_RENAME_FROM_DB={ clients:{start_date:'date'}, holdings:{created_at:'addedAt'} };

function applyRename(obj,map){
  if(!map)return obj;
  const out={...obj};
  Object.keys(map).forEach(from=>{ if(from in out){ out[map[from]]=out[from]; delete out[from]; } });
  return out;
}
...
function fromSnakeRowForTable(table,o){
  ...
  const remapped={...o};
  Object.keys(rev).forEach(dbCol=>{ if(dbCol in remapped){ remapped[rev[dbCol]]=remapped[dbCol]; delete remapped[dbCol]; } });
...
async function pushArrayToSupabase(table,rows){
  ...
  try{
    ...
    toast(`⚠️ Could not sync ${table} to the cloud — this change is only saved on this device right now`,'r');
  }
}
...
function sv(k,d){
  ...
  else if(SHARED_BLOB_KEYS.has(k))pushBlobToSupabase(k,d);
}

// Pulls the shared clients/holdings/transactions/uploads data from Supabase
// and replaces local state with it. Called on every page load (see
// checkAuth below) so any employee's uploads show up everywhere.
// SAFETY RULE (added after a real data-loss incident): if the cloud table
// comes back empty but this device already has local data for it, that
// almost always means this device's data was never pushed up yet — NOT
// that someone intentionally emptied it. In that case we push local data
// UP to Supabase instead of wiping it with the empty cloud result. Cloud
// only "wins" and overwrites local when it actually has rows to offer.
function reconcileRow(table, cloudRows, localVar, svKey){
  if((!cloudRows||cloudRows.length===0) && localVar.length>0){
    console.warn(`${table}: cloud is empty but local has ${localVar.length} row(s) — pushing local up instead of wiping it.`);
    pushArrayToSupabase(table, localVar);
    return localVar;
  }
  return (cloudRows||[]).map(r=>fromSnakeRowForTable(table,r));
}

async function pullClientDataFromSupabase(opts={}){
  const {silent=false}=opts;
  if(typeof sb==='undefined')return;
  try{
    const [c,h,t,u,s]=await Promise.all([
      sb.from('clients').select('*'),
      sb.from('holdings').select('*'),
      sb.from('transactions').select('*'),
      sb.from('uploads').select('*'),
      sb.from('shared_state').select('*'),
    ]);
    _suppressSupaPush=true;
    clients=reconcileRow('clients',c,clients); sv(K.C,clients);
    holdings=reconcileRow('holdings',h,holdings); sv(K.H,holdings);
    txns=reconcileRow('transactions',t,txns); sv(K.T,txns);
    uploads=reconcileRow('uploads',u,uploads); sv(K.UP,uploads);
    _lastSyncedIds={
      clients:new Set(clients.map(x=>x.id)),
      holdings:new Set(holdings.map(x=>x.id)),
      transactions:new Set(txns.map(x=>x.id)),
      uploads:new Set(uploads.map(x=>x.id)),
    };
    // Same safety rule for the blob keys: if a key has NO row at all in
    // shared_state but this device already has non-default local content
    // for it, push local up rather than resetting it to empty/default.
    const seenKeys=new Set((s||[]).map(row=>row.key));
    const blobDefault={i72_analysis:[],i72_pms_sector_alloc:{},i72_tk:{},i72_snaps:[],i72_fs_state:{},i72_fsi_manual:[]};
    const blobLocal={
      i72_analysis: ld(K.AN),
      i72_pms_sector_alloc: pmsSectorAlloc,
      i72_tk: tickerMap,
      i72_snaps: JSON.parse(localStorage.getItem('i72_snaps')||'[]'),
      i72_fs_state: JSON.parse(localStorage.getItem('i72_fs_state')||'{}'),
      i72_fsi_manual: JSON.parse(localStorage.getItem('i72_fsi_manual')||'[]'),
    };
    Object.keys(blobDefault).forEach(key=>{
      const local=blobLocal[key];
      const localIsNonEmpty=Array.isArray(local)?local.length>0:Object.keys(local||{}).length>0;
      if(!seenKeys.has(key) && localIsNonEmpty){
        console.warn(`shared_state:${key} missing in cloud but local has data — pushing local up instead of wiping it.`);
        pushBlobToSupabase(key, local);
      }
    });
    (s||[]).forEach(row=>{
      const val=row.value;
      switch(row.key){
        case 'i72_analysis': sv(K.AN, val||[]); break;
        case 'i72_pms_sector_alloc': pmsSectorAlloc=val||{}; sv('i72_pms_sector_alloc', pmsSectorAlloc); break;
        case 'i72_tk': tickerMap=val||{}; sv('i72_tk', tickerMap); break;
        case 'i72_snaps': sv('i72_snaps', val||[]); break;
        case 'i72_fs_state': sv('i72_fs_state', val||{}); break;
        case 'i72_fsi_manual': sv('i72_fsi_manual', val||[]); break;
      }
    });
    _suppressSupaPush=false;
    if(typeof populateDropdowns==='function')populateDropdowns();
    if(typeof renderAll==='function')renderAll();
    if(typeof renderTickerMappings==='function')renderTickerMappings();
    if(!silent)toast('Data refreshed from cloud ✓');
  }catch(e){
    _suppressSupaPush=false;
    console.error('pullClientDataFromSupabase failed:',e);
    if(!silent)toast('Could not load shared data from Supabase — showing this device\'s local copy','r');
  }
}
...
const NSE_SYM={
  ...
  'DIFFUSION ENGINEERS LTD':'DIFFNKG','RAYMOND REALTY LTD':'RAYMONDREL','JAYASWAL NECO INDUSTRIES LTD':'JAYNECOIND',
  'GALA PRECISION ENGINEERING LTD':'GALAPREC','ENTERO HEALTHCARE SOLUTIONS LTD':'ENTERO','CHEMBOND CHEMICAL LTD':'CHEMBONDCH',
  'CHEMBOND CHEMICALS LTD':'CHEMBONDCH',
};
...
// Quietly re-pull client/holdings/transactions/uploads every 60s so a
// colleague's upload shows up on this screen without needing a manual
// refresh or full page reload.
setInterval(()=>{
  if(typeof pullClientDataFromSupabase==='function')pullClientDataFromSupabase({silent:true});
},60000);
...
// symbol list using the actual company name (progressively shortened), score results by how many
// significant words overlap with the company name, and return the best
// NSE cash-equity match.
    ...
    for(const term of candidates){
      try{
        ...
        if(data.status&&Array.isArray(data.data)&&data.data.length){
          ...
          if(eq.length){
            // Score by the LENGTH of matched words, not just the count — two
            // unrelated companies can share one short, generic word (e.g.
            // "VIP Industries" vs "VIP Clothing" both containing "VIP"), and
            // a plain word-count would treat that shared prefix as if it
            // were a confident match. Weighting by length means a genuinely
            // distinctive word like "INDUSTRIES" matters far more than a
            // 3-letter prefix nearly any related company could also share.
            const scored=eq.map(d=>{
              const dn=(d.name||d.tradingsymbol||'').toUpperCase();
              const matched=words.filter(w=>dn.includes(w));
              const score=matched.reduce((s,w)=>s+w.length,0);
              const longestMatch=matched.reduce((m,w)=>Math.max(m,w.length),0);
              return{d,score,longestMatch};
            }).sort((a,b)=>b.score-a.score);
            const top=scored[0];
            const runnerUp=scored[1];
            // Refuse to guess (rather than pick arbitrarily) when: nothing
            // scored, the only match was on a short/generic word (<=3
            // chars — exactly the "VIP" case), or two candidates are tied.
            // All three cases fall through to the AI-assisted lookup below,
            // which can actually tell "VIP Industries" and "VIP Clothing"
            // apart using real company knowledge, then gets safely verified
            // against Angel One's data either way.
            if(top&&top.score>0&&top.longestMatch>3&&(!runnerUp||runnerUp.score<top.score)){
              return{...top.d,exchange};
            }
          }
        }
...
// ==============================================================
// Calls the yahoo-ticker-lookup Edge Function — a real, verified answer
// (confirmed accurate via direct testing) rather than a guess, tried
// before ever falling back to Angel One's own limited search or an AI
// suggestion.
async function lookupYahooTicker(companyName){
  try{
    if(typeof sb === 'undefined' || !sb.token) return null;
    const resp = await fetch(sb.url + '/functions/v1/yahoo-ticker-lookup', {
      method: 'POST',
      headers: {
    ...
    if(!resp.ok || !data.found) return null;
    return { symbol: data.symbol, exchange: data.exchange };
  } catch(e){
    console.error('Yahoo ticker lookup failed (non-fatal, falling back):', e);
    return null;
  }
}
...
async function fetchLivePrices(){
  ...
  holdings.forEach(h=>{
    if(isCorruptedHolding(h)){skippedCorrupted++;return}
    const s=(h.stock||'').trim();
    if(s)stockMap[s]=resolveNSE(s);
  });
  const entries=Object.entries(stockMap);
  if(!entries.length){toast('No holdings to fetch prices for','r');return}

  statusEl.textContent=`⏳ Fetching prices for ${entries.length} stocks via Angel One...${skippedCorrupted?` (skipped ${skippedCorrupted} flagged as bad data)`:''}`;
  ...
  async function fetchOne(stockName,nseSym,attempt,cacheRetried){
    let tradingSym=nseSym+'-EQ';
    let token,exchange='NSE';

    // No confirmed ticker mapping for this stock — auto-discover it by
    // checking Yahoo Finance's real company-name search first (verified
    // accurate via direct testing — correctly resolves even non-obvious
    // tickers), then falling back to Angel One's own search, then an
    // AI-suggested guess as a last resort. Whichever succeeds gets
    // remembered permanently so this never has to happen again for this
    // stock, for anyone.
    if(!isKnownSymbolMapping(stockName)){
      let found=null;
      let source=null;

      const yahooMatch=await lookupYahooTicker(stockName);
      if(yahooMatch&&yahooMatch.symbol){
        const confirmedSym=yahooMatch.symbol+'-EQ';
        const tokenResult=await resolveSymbolToken(confirmedSym,headers,tokenCache,0,yahooMatch.exchange);
        if(tokenResult){
          found={tradingsymbol:confirmedSym,symboltoken:tokenResult.token,exchange:tokenResult.exchange};
          source='yahoo';
        }
      }

      if(!found){
        found=await smartSearchTicker(stockName,headers);
        if(found)source='search';
      }
      if(!found){
      ...
      if(found){
        ...
        autoDiscovered++;
        if(source==='yahoo')console.log(`Yahoo Finance ticker match for "${stockName}" → ${discoveredTicker}`);
        else if(source==='ai')console.log(`AI-assisted ticker match for "${stockName}" → ${discoveredTicker} (verified against Angel One)`);
      }else{
    ...
    try{
      ...
      // AB4046 and similar "token not found" responses mean the specific
      // ticker/token cached for THIS stock is stale or was wrong to begin
      // with (e.g. an earlier auto-guess) — not a session problem. Clear
      // just this stock's cached entries and retry once with a fully fresh
      // discovery, instead of repeating the same broken guess on every
      // future fetch forever.
      if(!cacheRetried&&(data.errorcode==='AB4046'||/token not found|symbol token/i.test(data.message||''))){
        delete tokenCache[exchange+':'+tradingSym];
        delete tickerMap[normalizeSpaces(stockName).toUpperCase()];
        sv(K.TK,tickerMap);
        console.log(`Stale cached ticker for "${stockName}" cleared, re-resolving fresh...`);
        return fetchOne(stockName,nseSym,0,true);
      }
  ...
  const summary = `✓ ${updated}/${entries.length} prices updated (${now})`
    + (skippedCorrupted ? ` · ${skippedCorrupted} bad-data holding${skippedCorrupted!==1?'s':''} skipped` : '')
...
function saveFsState(s){sv(FS_STATE_KEY,s)}
...
function manualSave(d){ sv(MANUAL_KEY, d); }
